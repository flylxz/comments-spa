...
- [ ] Top-level комментарии отображаются в `<table>` с сортируемыми заголовками
- [ ] Preview работает без перезагрузки страницы
- [ ] Backend использует OOP-классы для сервисов/репозиториев
