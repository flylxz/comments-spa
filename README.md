...
| CAPTCHA         | JWT-подпись (TTL 3 мин, `JWT_SECRET`); regex `[a-zA-Z0-9]+` из shared; rate limit 30/мин на `/api/captcha`                |
| Валидация ввода | Zod в `@comments-spa/shared` + клиентские расширения на frontend; `userName` и `captchaAnswer` — `[a-zA-Z0-9]+`         |
