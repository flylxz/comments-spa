...
**CAPTCHA:** `GET /api/captcha` возвращает SVG и подписанный JWT `captchaId` (TTL 3 мин). При `POST /api/comments` токен проверяется в `CaptchaService.verifyCaptcha()`. Ответ должен совпадать с `[a-zA-Z0-9]+`.

> **Известное ограничение:** ответ сейчас хранится в JWT payload (читается без секрета). План исправления — [уровень 3b в ROADMAP](docs/ROADMAP.md#уровень-3b--security-hardening-оставшееся).
...
| Угроза          | Мера                                                                                                                       |
| --------------- | -------------------------------------------------------------------------------------------------------------------------- |
| XSS             | `sanitize-html` (backend) + DOMPurify (frontend); whitelist тегов `a`, `code`, `i`, `strong`; `validateCommentHtml`; `href` только `http`/`https`/`mailto`; auto `target="_blank"` + `rel="noopener noreferrer"` |
| SQL injection   | Prisma ORM, без raw queries                                                                                                |
| Upload abuse    | Whitelist MIME/расширений; TXT ≤ 100 KB; изображения ≤ 5 MB; валидация через `sharp`, resize до 320×240                  |
| CAPTCHA         | JWT-подпись (TTL 3 мин, `JWT_SECRET`); regex `[a-zA-Z0-9]+` на клиенте и сервере; rate limit 30/мин на `/api/captcha`    |
| Валидация ввода | Zod на backend и frontend; `userName` и `captchaAnswer` — `[a-zA-Z0-9]+`                                                  |
| Rate limiting   | `express-rate-limit`: GET `/api/captcha` — 30/мин, POST `/api/comments` — 10/мин на IP                                    |
...
### Планируется (уровень 3b)

Подробный план с оценками — [docs/ROADMAP.md](docs/ROADMAP.md#уровень-3b--security-hardening-оставшееся).

| Угроза | Планируемая мера |
| ------ | ---------------- |
| CAPTCHA bypass | Server-side store (ответ не в JWT payload); one-time token после успешного POST |
| Небезопасный `homePage` | Whitelist схем `http`/`https` на backend |
| DoS / scraping | `.max()` на поля формы; rate limit на `GET /api/comments` |
| Upload (defense in depth) | Всегда re-encode изображений через `sharp` перед сохранением |
