...
Перед настройкой **запустите Docker Desktop** (или другой Docker daemon) — без него PostgreSQL не поднимется.
...
task dev     # frontend :5173 + backend :3000 (браузер откроется автоматически)
```

Если `task setup` упал на шаге с Postgres, сначала убедитесь, что Docker запущен, затем:
...
В `frontend/.env` используйте `VITE_API_URL=/api` — так запросы идут через proxy и CORS не нужен на любом порту Vite.
...
```bash
task db:seed   # 50 top-level комментариев
```
...
| -------------- | --------------------------------- | ---------------------------------------------------------------------- |
| `DATABASE_URL` | PostgreSQL connection string      | `postgresql://user:password@localhost:5433/comments_spa?schema=public` |
...
| Переменная        | Описание                                      | Пример                      |
| ----------------- | --------------------------------------------- | --------------------------- |
| `VITE_API_URL`    | Базовый URL API                               | `/api` (локально через Vite proxy) |
| `VITE_API_ORIGIN` | Origin для статики `/uploads/*` (опционально) | не нужен при `/api`                |
...
Локальный PostgreSQL поднимается через `docker-compose.yml` на порту **5433** (чтобы не конфликтовать с локальным PostgreSQL на 5432). **Docker Desktop (или daemon) должен быть запущен.**

Если `backend/.env` уже существует с портом `5432`, обновите `DATABASE_URL` на `5433`.
